...
export class EngineBridge {
  ...
  private _handleGoToCard(payload: EngineEvent & { type: "GoToCard" } extends { payload: infer P } ? P : never): void {
    if (!this._onCardChange) return;
    // The WildCardApp resolves the direction/cardId to an actual index.
    // Here we just forward the request.
    // Store the payload so WildCardApp can read it.
  ...
  private _handleSetField(payload: { fieldId: string; content: string }): void {
    ...
    const updatedObjects = state.objects.map((obj) => {
      if (obj.type === "field" && (obj.name === payload.fieldId || obj.id === payload.fieldId)) {
  ...
  private _handleSetProperty(payload: {
    ...
    const state = this._renderer.state;
    const updatedObjects = state.objects.map((obj) => {
      if (obj.name === payload.objectId || obj.id === payload.objectId) {
